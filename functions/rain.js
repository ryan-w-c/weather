function init() {
  ...
  function stopAnimation() {
    ...
    // Remove the renderer's DOM element (the canvas)
    if (renderer && renderer.domElement) {
      renderer.domElement.parentNode.removeChild(renderer.domElement);
    }

    // Clean up Three.js objects to avoid memory leaks (optional but recommended)
    scene.dispose();
    renderer.dispose();
    camera = null;
    scene = null;
    ...
    flash = null;
    rain = null;
    rainGeo.dispose(); // Dispose of the geometry
    rainGeo = null;
  }
  initSelf();
}
